...
const asdf: React.FC<PlayerProps> = ({ }) => {
    ...
    useEffect(() => {
        ...
        // 컴포넌트가 마운트될 때만 실행되고, 언마운트될 때 정리 함수가 호출됨
        return () => {
            setIsLoading(false); // 언마운트 시 로딩 상태 초기화
        };
    ...
    useEffect(() => {
        ...
        if (!existingScript) {
            ...
            initializePlayer(); // 이미 스크립트가 로드된 경우 플레이어를 초기화
        }
    ...
    const initializePlayer = () => {
        ...
        if (!videoRef.current) {
            ...
            return; // 요소가 마운트되지 않았으므로 초기화를 중지합니다.
        }
    ...
    useEffect(() => {
        const initializePlayerIfMounted = () => {
            if (videoRef.current) {
                initializePlayer(); // videoRef가 존재할 때만 initializePlayer 함수 호출
            } else {
        ...
        // videoRef가 변경될 때마다 initializePlayerIfMounted 함수를 호출
    }, [videoRef.current]);
